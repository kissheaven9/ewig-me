...
// Разрешённые источники (кто может звать прокси). Добавь свой домен, если появится.
const ALLOWED = [
  "https://kissheaven9.github.io"
];
...
export default {
  async fetch(request, env) {
    ...
    const answers = Array.isArray(body.answers)
      ? body.answers.slice(0, 12).map((a) => String(a == null ? "" : a).slice(0, 800))
      : [];

    const langName = lang === "ru" ? "русском" : "немецком (Deutsch)";
    const sys =
      `Ты пишешь уважительные, тёплые биографии для мемориальной страницы на ${langName} языке. ` +
      `Верни СТРОГО JSON вида {"formal":"...","personal":"..."}. ` +
      `"formal" — достойная, энциклопедичная биография (3–4 абзаца, абзацы разделяй символами \\n\\n). ` +
      `"personal" — тёплый, личный текст от лица семьи (3–4 абзаца). ` +
      `Опирайся только на предоставленные ответы, не выдумывай фактов. Тон уважительный, без пафоса и сенсаций.`;
    const user =
      `Имя: ${name || "(не указано)"}\n\nОтветы на вопросы о человеке:\n` +
